...
@register_loss("WGFormer")
class WGFormerLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        loss = (
            distance_loss + self.args.coord_loss * coord_loss
        )
        logging_output = {
            "loss": loss.data,
        ...
        if not self.training:
            logging_output["idx_name"] = sample["idx_name"]

        return loss, 1, logging_output

    def compute_loss(self, model, net_output, sample, reduce=True):
        distance_predict, coord_predict = net_output[0], net_output[-1]
        token_mask = sample["net_input"]["src_tokens"].ne(self.padding_idx) 
        ...
        coord_target = sample["target"]["coord_target"]
